...
function InRepairProductsScreen() {
  ...
  // console.log(response.data.data.filter((product)=>product.history[0].status[0].name==='in repair'))
      setLoading(false);
    } catch (error) {
  ...
  return (
    <div className="m-5">
      <div>
        <h1 className="text-3xl font-semibold text-neutral-900">In Repair</h1>
      ...
      <div className="border rounded-md border-neutral-700 flex flex-col">
        <div className="overflow-auto max-h-[calc(100vh-250px)] relative">
          <table className="table-auto w-full border-collapse">
            <thead className="border-b text-left sticky top-0 bg-white z-10 shadow-sm">
              <tr>
            ...
            <tbody>
              {isLoading ? (
                ...
                products.filter((product)=>product.history[0].status[0].name==='in repair').map((product, idx) => (
                  <ProductRow
                    key={product._id}
                    index={idx}
                    product={product}
    ...
    <tr className="border-b hover:bg-teal-50 hover:text-teal-700">
      ...
      <td className="px-4 py-2 text-neutral-700 font-semibold text-sm">
        {user._id === product.createdBy ? (
          <NavLink
            to={`edit/${product._id}`}
            className="px-4 py-1 bg-neutral-800 text-slate-100 text-sm rounded-md hover:bg-neutral-600 hover:scale-95 transition-transform whitespace-nowrap inline-block"
          >
            Edit
          </NavLink>
        ) : (
          <span className="px-4 py-1 bg-gray-200 text-gray-500 text-sm rounded-md cursor-not-allowed whitespace-nowrap inline-block">
            Read Only
          </span>
        )}
      </td>
...
export default InRepairProductsScreen;
